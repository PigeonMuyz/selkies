...
#selkies-sidebar-panel.open {
  right: 0;
}
#selkies-sidebar-panel .panel-header {
...
#selkies-sidebar-panel .panel-header h3 {
  margin: 0;
  font-size: 16px;
  font-weight: 600;
  color: #fff;
...
#selkies-sidebar-panel .panel-close {
  width: 28px;
  height: 28px;
  border-radius: 50%;
  border: none;
...
#selkies-sidebar-panel .panel-close:hover {
  background: rgba(255, 255, 255, 0.15);
  color: #fff;
}
#selkies-sidebar-panel .panel-section {
  padding: 12px 16px;
  border-bottom: 1px solid rgba(255, 255, 255, 0.04);
}
#selkies-sidebar-panel .panel-section-title {
  font-size: 11px;
...
#selkies-sidebar-panel .panel-item {
  display: flex;
  justify-content: space-between;
  align-items: center;
  padding: 8px 0;
...
#selkies-sidebar-panel .panel-item label {
  color: rgba(255, 255, 255, 0.75);
}
#selkies-sidebar-panel .panel-item .value {
  color: #fff;
  font-weight: 500;
}
#selkies-sidebar-panel .panel-item .status-dot {
...
#selkies-sidebar-panel .panel-item .status-dot.connected { background: #4caf50; }
#selkies-sidebar-panel .panel-item .status-dot.disconnected { background: #f44336; }
#selkies-sidebar-panel .panel-item .status-dot.connecting { background: #ff9800; }
#selkies-sidebar-panel .panel-btn {
  width: 100%;
  padding: 10px;
  border-radius: 8px;
...
#selkies-sidebar-panel .panel-btn:hover {
  background: rgba(255, 255, 255, 0.12);
}
#selkies-sidebar-overlay {
  position: fixed;
...
const initializeUI = () => {
  ...
  sidebarOverlay.id = 'selkies-sidebar-overlay';
  document.body.appendChild(sidebarOverlay);

  const sidebarPanel = document.createElement('div');
  sidebarPanel.id = 'selkies-sidebar-panel';
  sidebarPanel.innerHTML = `
    ...
    <div class="panel-section">
      <div class="panel-section-title">连接状态</div>
      <div class="panel-item">
        <label><span class="status-dot connecting" id="ws-status-dot"></span>WebSocket</label>
        <span class="value" id="ws-status-text">连接中...</span>
      </div>
      <div class="panel-item">
        <label>FPS</label>
        <span class="value" id="panel-fps">0</span>
      </div>
    </div>
    <div class="panel-section">
      ...
      <div class="panel-item">
        <label>帧率</label>
        <span class="value" id="panel-framerate">${framerate}</span>
      </div>
      <div class="panel-item">
        <label>编码器</label>
        <span class="value" id="panel-encoder">${currentEncoderMode}</span>
      </div>
      <div class="panel-item">
        <label>分辨率模式</label>
        <span class="value">${window.is_manual_resolution_mode ? '手动' : '自动'}</span>
      </div>
    </div>
    <div class="panel-section">
      <div class="panel-section-title">快捷操作</div>
      <button class="panel-btn" id="panel-fullscreen-btn">全屏</button>
      <button class="panel-btn" id="panel-clipboard-btn">同步剪贴板</button>
    </div>
  `;
  ...
  function toggleSidebarPanel() {
    ...
    sidebarPanel.classList.toggle('open', sidebarOpen);
    sidebarOverlay.classList.toggle('open', sidebarOpen);
    // 通知 input 层侧边栏状态
    window.postMessage({ type: 'sidebarVisibilityChanged', isOpen: sidebarOpen }, window.location.origin);
  }
  function closeSidebarPanel() {
    sidebarOpen = false;
    sidebarPanel.classList.remove('open');
    sidebarOverlay.classList.remove('open');
    window.postMessage({ type: 'sidebarVisibilityChanged', isOpen: false }, window.location.origin);
  }

  sidebarOverlay.addEventListener('click', closeSidebarPanel);
  document.getElementById('panel-close-btn').addEventListener('click', closeSidebarPanel);
  document.getElementById('panel-fullscreen-btn').addEventListener('click', () => {
    enterFullscreen();
    closeSidebarPanel();
  });
  document.getElementById('panel-clipboard-btn').addEventListener('click', () => {
    enableClipboard();
    closeSidebarPanel();
  });
  ...
  // 定时更新面板信息
  setInterval(() => {
    const fpsEl = document.getElementById('panel-fps');
    if (fpsEl) fpsEl.textContent = window.fps || 0;
